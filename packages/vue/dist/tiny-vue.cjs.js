...
function parseInterpolation(context) {
    ...
    let closeIndex = context.source.indexOf(closeDelimiter, openDelimiter.length);
    // {{
    ...
    function patchKeyedChildren(c1, c2, container, parentComponent, parentAnchor) {
        ...
        if (i > e1) {
            if (i <= e2) {
                ...
                for (let j = e2; j >= i; j--) {
                    ...
                    // mount
                    patch(null, nextChild, container, parentComponent, nextAnchor);
                }
            }
        }
        ...
        // a b (d c y e) f g
            ...
            for (let j = s2; j <= e2; j++) {
                ...
                if (nextChild.key != null) {
                    if (keyToNewIndexMap.has(nextChild.key)) {
                        console.warn(`Duplicate keys found during update:`, JSON.stringify(nextChild.key), `Make sure keys are unique.`);
                    }
            ...
            for (let k = toBePatched - 1; k >= 0; k--) {
                ...
                const nextAnchor = newIndex + 1 < l2 ? c2[newIndex + 1].el : null;
